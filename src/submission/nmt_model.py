...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.c_projection = nn.Linear(2 * hidden_size, hidden_size, bias=False,device=dev)
        #(Linear Layer with no bias), called W_{c} in the PDF.
        self.att_projection = nn.Linear(2 * hidden_size, hidden_size, bias=False,device=dev)
        #(Linear Layer with no bias), called W_{attProj} in the PDF.
        self.combined_output_projection = nn.Linear(3 * hidden_size, hidden_size, bias=False, device=dev)
        #(Linear Layer with no bias), called W_{u} in the PDF.
        self.target_vocab_projection  = nn.Linear( hidden_size,len(vocab.tgt), bias=False, device=dev)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X_p = pack_padded_sequence(X, torch.Tensor(source_lengths))
        
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X_p)
        enc_hiddens, len_unpk = pad_packed_sequence(enc_hiddens, batch_first=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        assert enc_hiddens_proj.shape == (enc_hiddens.shape[0], enc_hiddens.shape[1], self.hidden_size)
        
        Y = self.model_embeddings.target(target_padded)
        assert Y.shape == (target_padded.shape[0], target_padded.shape[1], self.model_embeddings.embed_size)
        
        for Y_t in torch.split(Y, 1,0):
            
            Y_t = torch.squeeze(Y_t, 0)
            assert Y_t.shape == (target_padded.shape[1], self.model_embeddings.embed_size)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        U_t = torch.cat((next_hidden, a_t), dim=1)
